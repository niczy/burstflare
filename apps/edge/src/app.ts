...
export function createApp(options: any = {}): { fetch(request: Request): Promise<Response> } {
  ...
  async function waitForTemplateBuildSuccess(token: string, buildId: string, maxAttempts = 30): Promise<any> {
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      if (build?.status === "failed" || build?.status === "dead_lettered") {
        throw createHttpError(`Template build ${buildId} failed with status ${build.status}`, 409);
      }
  ...
  const routes = [
    ...
    {
      method: "POST",
      pattern: "/api/quickstart/session",
      handler: withErrorHandling(async (request) => {
        const token = requireToken(request, service);
        if (!token) {
          return unauthorized();
        ...
        const body = rawBody ? await parseJson(rawBody) : {};
        ...
        const sessionName =
          typeof body.sessionName === "string" && body.sessionName.trim()
            ...
            : quickName("session");
        const version =
          typeof body.version === "string" && body.version.trim() ? body.version.trim() : "1.0.0";

        const templateResult = await service.createTemplate(token, {
          name: templateName,
          description:
            typeof body.templateDescription === "string" && body.templateDescription.trim()
              ? body.templateDescription.trim()
              : "Quick-start template generated by BurstFlare"
        });

        const templateId = templateResult.template.id;
        const templateVersionResult = await service.addTemplateVersion(token, templateId, {
          version,
          manifest: {
            image: `registry.cloudflare.com/example/${templateId}:${version}`,
            features: ["ssh", "browser", "snapshots"],
            persistedPaths: ["/workspace"]
          }
        });

        if (!templateVersionResult.build?.id) {
          throw createHttpError("Template build id missing from quick-start flow", 502);
        }

        await waitForTemplateBuildSuccess(token, templateVersionResult.build.id);
        const promoteResult = await service.promoteTemplateVersion(
          token,
          templateId,
          templateVersionResult.templateVersion.id
        );

        const sessionResult = await service.createSession(token, {
          name: sessionName,
          templateId
        });
        const started = await transitionSession(token, sessionResult.session.id, "start");

        return toJson({
          template: templateResult.template,
          templateVersion: templateVersionResult.templateVersion,
          release: promoteResult.release || null,
